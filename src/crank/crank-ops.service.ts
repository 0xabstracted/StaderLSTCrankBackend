...
export class CrankOpsService {
  ...
  constructor(
    ...
    this.stateAccountPublicKey = this.solanaUtilService.getStateAccountPublicKey();
    this.validatorsListPublicKey = this.solanaUtilService.getValidatorListPublicKey();
    this.stakeListPublicKey = this.solanaUtilService.getStakeListPublicKey();
    this.reservePda = this.solanaUtilService.getReservePda();
    this.stakeDepositAuthority = this.solanaUtilService.getStakeDepositAuthority();
    this.stakeWithdrawAuthority = this.solanaUtilService.getStakeWithdrawAuthority();
    this.operationalSolAccountPublicKey = this.solanaUtilService.getOperationalSolAccountPublicKey();
    this.staderSolMintPublicKey = this.solanaUtilService.getStaderSolMintPublicKey();
    this.staderSolMintAuthority = this.solanaUtilService.getStaderSolMintAuthority();
    this.treasuryStaderSolAccount = this.solanaUtilService.getTreasuryStaderSolAccount();
  }
  ...
  async stakeReserve() {
    try {
      ...
      for (let i = 0; i < validators.length; i++) {
        ...
        console.log(`Successfully processed stake account ${i} of ${validators.length} for stakeReserve: ${stakeAccountKeypair.publicKey}`);
      }
  ...
  async stakeReserveCall(stakeAccountKeypair: Keypair, stakeReserveParam: StakeReserveParam){
    const {
      validatorIndex,
      validatorVote
  } = stakeReserveParam
    const tx = await this.program.methods.stakeReserve(validatorIndex)
      .accounts({
  ...
  async updateActive() {
    try {
      ...
      while (hasMoreDelegations) {
        const stakeDelegations = await this.crankDataService.getStakeDelegationsList({
          page,
          limit
        });
        ...
        console.log(`Processing batch ${page}, delegations: ${stakeDelegations.data.length}`);

        // Process current batch
        for (const delegation of stakeDelegations.data) {
          console.log(`Processing delegation for validator: ${delegation.validatorAccount}`);
          ...
          processedCount++;
          console.log(`Successfully processed stake account ${processedCount} of ${stakeDelegations.total} for updateActive: ${delegation.stakeAccount}`);
        }
  ...
  async updateActiveCall(stakeAccount: PublicKey, updateActiveParam: UpdateActiveParam){
    const {
      stakeIndex,
      validatorIndex
    } = updateActiveParam
    const tx = await this.program.methods.updateActive(stakeIndex, validatorIndex)
    .accounts({
      state: this.stateAccountPublicKey,
      stakeList: this.stakeListPublicKey,
      validatorList: this.validatorsListPublicKey,
    ...
    try {
      ...
      while (hasMoreDelegations) {
        ...
        for (const delegation of stakeDelegations.data) {
          ...
          processedCount++;
          console.log(`Successfully processed stake account ${processedCount} of ${stakeDelegations.total} for updateDeactivated: ${delegation.stakeAccount}`);
        }
  ...
  async updateDeactivatedCall(stakeAccount: PublicKey, updateDeactivatedParam: UpdateDeactivatedParam){
    const {
      stakeIndex
    } = updateDeactivatedParam

    const tx = await this.program.methods.updateDeactivated(stakeIndex)
      .accounts({
        state: this.stateAccountPublicKey,
        stakeList: this.stakeListPublicKey,
        stakeAccount: stakeAccount,
    ...
    try {
      ...
      while (hasMoreDelegations) {
        ...
        for (const delegation of stakeDelegations.data) {
          ...
          console.log(`Successfully processed stake account ${processedCount} of ${stakeDelegations.total} for deactivateStake: ${delegation.stakeAccount}`);
        }
  ...
  async deactivateStakeCall(stakeAccount: PublicKey, deactivateStakeParam: DeactivateStakeParam){
    const {
      stakeIndex,
      validatorIndex,
      splitStakeAccount
    ...
    const tx = await this.program.methods.deactivateStake(stakeIndex, validatorIndex)
        .accounts({
            state: this.stateAccountPublicKey,
            reservePda: this.reservePda,
            validatorList: this.validatorsListPublicKey,
    ...
    try {
      ...
      while (hasMoreDelegations) {
        ...
        for (const delegation of stakeDelegations.data) {
          ...
          console.log(`Successfully processed stake account ${processedCount} of ${stakeDelegations.total} for mergeStake: ${delegation.stakeAccount}`);
        }
  ...
  async mergeStakeCall(stakeAccount: PublicKey, mergeStakeParam: MergeStakeParam){
    const {
    ...
    const tx = await this.program.methods.mergeStakes(
      destinationStakeIndex, 
      sourceStakeIndex, 
      validatorIndex
    ).accounts({
    ...
    try {
      ...
      while (hasMoreDelegations) {
        ...
        for (const delegation of stakeDelegations.data) {
          ...
          console.log(`Successfully processed stake account ${processedCount} of ${stakeDelegations.total} for redelegate: ${delegation.stakeAccount}`);
        }
  ...
  async redelegateCall(stakeAccount: PublicKey, redelegateParam: RedelegateParam){
    const {
      stakeIndex,
    ...
    const tx = await this.program.methods.redelegate(
      stakeIndex, 
      sourceValidatorIndex, 
      destValidatorIndex
    ).accounts({
