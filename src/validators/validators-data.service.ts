...
export class ValidatorsDataService{
  ...
  async createOrUpdateBulk(validators:any[]){
    try{  
      ...
      const existingMap = new Map(existingValidators.map(ev => [ev.voteAccount, ev]));
      const toInsert = [];
      const toUpdate = [];
      ...
      for (const data of validators) {
        ...
        if (existing) {
          const merged = this.validatorRepository.merge(existing, this.mapToEntity(data));
      ...
      throw new HttpException('Error while updating validators data',HttpStatus.BAD_REQUEST,{
        cause:error
      });
    }
  ...
  async createValidator(validatorData:any){
    try{
       const newValidator = this.validatorRepository.create(this.mapToEntity(validatorData));
    return await this.validatorRepository.save(newValidator);}
    catch(error){
      throw new HttpException('Error while storing validator details',HttpStatus.BAD_REQUEST,{
        cause:error
      })
    }
  }
