...
export class UnstakeTicketListener implements OnModuleInit, OnModuleDestroy {
  ...
  private async setupUnstakeTicketListener() {
    try {
      ...
      this.listenerSubscriptionId = program.addEventListener(
        'OrderUnstakeEvent',
        async (event) => {
          try {
            this.logger.log(
              `Unstake ticket created event received: ${JSON.stringify(event)}`,
            );
            ...
            this.logger.log(
              `Unstake ticket saved to database: ${ticketPublicKey}`,
            );
          } catch (error) {
            this.logger.error(
              `Error processing unstake ticket event: ${error.message}`,
              error.stack,
            );
          }
        },
      ...
      this.logger.log(
        `Unstake ticket listener setup complete with subscription ID: ${this.listenerSubscriptionId}`,
      );
    } catch (error) {
      this.logger.error(
        `Error setting up unstake ticket listener: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async onModuleDestroy() {
    if (this.listenerSubscriptionId) {
      ...
      this.logger.log(
        `Removed unstake ticket listener with subscription ID: ${this.listenerSubscriptionId}`,
      );
    }
  }
}
