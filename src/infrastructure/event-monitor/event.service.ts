...
export class SolanaEventsService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    // Admin Events
    @InjectModel(ChangeAuthorityEvent.name) private changeAuthorityEventModel: Model<ChangeAuthorityEventDocument>,
    @InjectModel(ConfigLpEvent.name) private configLpEventModel: Model<ConfigLpEventDocument>,
    @InjectModel(ConfigStaderLiquidStakingEvent.name) private configStaderLiquidStakingEventModel: Model<ConfigStaderLiquidStakingEventDocument>,
    @InjectModel(EmergencyPauseEvent.name) private emergencyPauseEventModel: Model<EmergencyPauseEventDocument>,
    @InjectModel(InitializeEvent.name) private initializeEventModel: Model<InitializeEventDocument>,
    @InjectModel(ReallocStakeListEvent.name) private reallocStakeListEventModel: Model<ReallocStakeListEventDocument>,
    @InjectModel(ReallocValidatorListEvent.name) private reallocValidatorListEventModel: Model<ReallocValidatorListEventDocument>,
    @InjectModel(ResumeEvent.name) private resumeEventModel: Model<ResumeEventDocument>,
    // Crank Events
    @InjectModel(DeactivateStakeEvent.name) private deactivateStakeEventModel: Model<DeactivateStakeEventDocument>,
    @InjectModel(MergeStakesEvent.name) private mergeStakesEventModel: Model<MergeStakesEventDocument>,
    @InjectModel(RedelegateEvent.name) private redelegateEventModel: Model<RedelegateEventDocument>,
    @InjectModel(StakeReserveEvent.name) private stakeReserveEventModel: Model<StakeReserveEventDocument>,
    @InjectModel(UpdateActiveEvent.name) private updateActiveEventModel: Model<UpdateActiveEventDocument>,
    @InjectModel(UpdateDeactivatedEvent.name) private updateDeactivatedEventModel: Model<UpdateDeactivatedEventDocument>,
    // Delayed Unstake Events
    @InjectModel(ClaimEvent.name) private claimEventModel: Model<ClaimEventDocument>,
    @InjectModel(OrderUnstakeEvent.name) private orderUnstakeEventModel: Model<OrderUnstakeEventDocument>,
    // Liq Pool Events
    @InjectModel(AddLiquidityEvent.name) private addLiquidityEventModel: Model<AddLiquidityEventDocument>,
    @InjectModel(LiquidUnstakeEvent.name) private liquidUnstakeEventModel: Model<LiquidUnstakeEventDocument>,
    @InjectModel(RemoveLiquidityEvent.name) private removeLiquidityEventModel: Model<RemoveLiquidityEventDocument>,
    // Management Events
    @InjectModel(AddValidatorEvent.name) private addValidatorEventModel: Model<AddValidatorEventDocument>,
    @InjectModel(RemoveValidatorEvent.name) private removeValidatorEventModel: Model<RemoveValidatorEventDocument>,
    @InjectModel(SetValidatorScoreEvent.name) private setValidatorScoreEventModel: Model<SetValidatorScoreEventDocument>,
    // User Events
    @InjectModel(DepositEvent.name) private depositEventModel: Model<DepositEventDocument>,
    @InjectModel(DepositStakeAccountEvent.name) private depositStakeAccountEventModel: Model<DepositStakeAccountEventDocument>,
    @InjectModel(WithdrawStakeAccountEvent.name) private withdrawStakeAccountEventModel: Model<WithdrawStakeAccountEventDocument>,
  ...
  async subscribeToChangeAuthorityEvent() {
    try {
      const changeAuthoritySubId = this.program.addEventListener("ChangeAuthorityEvent", async (event) => {
        const newEvent = new this.changeAuthorityEventModel({
          ...
          treasuryStaderSolAccountChange: event.treasuryStaderSolAccountChange,
  ...
  async subscribeToConfigLpEvent() {
    try {
      const configLpSubId = this.program.addEventListener("ConfigLpEvent", async (event) => {
  ...
  async subscribeToConfigStaderLiquidStakingEvent() {
    try {
      const configStaderSubId = this.program.addEventListener("ConfigStaderLiquidStakingEvent", async (event) => {
        const newEvent = new this.configStaderLiquidStakingEventModel({
          ...
          withdrawStakeAccountEnabledChange: event.withdrawStakeAccountEnabledChange,
      ...
      console.error('Error subscribing to ConfigStaderLiquidStakingEvent:', error);
    }
  }

  async subscribeToEmergencyPauseEvent() {
    try {
      const emergencyPauseSubId = this.program.addEventListener("EmergencyPauseEvent", async (event) => {
  ...
  async subscribeToInitializeEvent() {
    try {
      const initializeSubId = this.program.addEventListener("InitializeEvent", async (event) => {
  ...
  async subscribeToReallocStakeListEvent() {
    try {
      const reallocStakeListSubId = this.program.addEventListener("ReallocStakeListEvent", async (event) => {
  ...
  async subscribeToReallocValidatorListEvent() {
    try {
      const reallocValidatorListSubId = this.program.addEventListener("ReallocValidatorListEvent", async (event) => {
  ...
  async subscribeToResumeEvent() {
    try {
      const resumeSubId = this.program.addEventListener("ResumeEvent", async (event) => {
  ...
  async subscribeToDeactivateStakeEvent() {
    try {
      const deactivateStakeSubId = this.program.addEventListener("DeactivateStakeEvent", async (event) => {
  ...
  async subscribeToMergeStakesEvent() {
    try {
      const mergeStakesSubId = this.program.addEventListener("MergeStakesEvent", async (event) => {
        const newEvent = new this.mergeStakesEventModel({
          ...
          lastUpdateDestinationStakeDelegation: event.lastUpdateDestinationStakeDelegation,
          sourceStakeIndex: event.sourceStakeIndex,
          sourceStakeAccount: event.sourceStakeAccount,
          lastUpdateSourceStakeDelegation: event.lastUpdateSourceStakeDelegation,
          validatorIndex: event.validatorIndex,
          validatorVote: event.validatorVote,
  ...
  async subscribeToRedelegateEvent() {
    try {
      const redelegateSubId = this.program.addEventListener("RedelegateEvent", async (event) => {
  ...
  async subscribeToStakeReserveEvent() {
    try {
      const stakeReserveSubId = this.program.addEventListener("StakeReserveEvent", async (event) => {
        ...
        newstakeDelegationEntry.stakeAcIndex = Number(event.stakeIndex.toString());
        newstakeDelegationEntry.validatorAccount = event.validatorVote.toString();
        newstakeDelegationEntry.validatorAcIndex = Number(event.validatorIndex.toString())
  ...
  async subscribeToUpdateActiveEvent() {
    try {
      const updateActiveSubId = this.program.addEventListener("UpdateActiveEvent", async (event) => {
        ...
        const stateMetricsEntry = this.stateMetricsRepository.create({
          ...
          delayedUnstakeCoolingDown: BigInt(state.stakeSystem.delayedUnstakeCoolingDown.toNumber()),
          emergencyCoolingDown: BigInt(state.emergencyCoolingDown.toNumber()),
          availableReserveBalance: BigInt(state.availableReserveBalance.toNumber()),
          validatorIndex: event.validatorIndex,
          validatorVote: event.validatorVote.toBase58(),
          validatorActiveBalance: BigInt(event.validatorActiveBalance.toNumber()),
          totalActiveBalance: BigInt(state.validatorSystem.totalActiveBalance.toNumber())
        });
        ...
        const oldStaderSolPrice = Number(event.staderSolPriceChange.old.toNumber()) / PRICE_DENOMINATOR;
        const newStaderSolPrice = Number(event.staderSolPriceChange.new.toNumber()) / PRICE_DENOMINATOR;
        const validatorActiveBalance = event.validatorActiveBalance.toString();
        const totalActiveBalance = event.totalActiveBalance.toString();
        const totalVirtualStakedLamports = event.totalVirtualStakedLamports.toString();
        const currentEpoch = Number(event.epoch);

        // Calculate the stake balance (extraLamports + newDelegatedLamports)
        const stakeBalanceWithoutRent = BigInt(event.extraLamports.toNumber()) + BigInt(event.delegationChange.new.toNumber());
        ...
        const priceHistoryEntry = this.staderSolPriceHistoryRepository.create({
          ...
          oldDelegatedLamports: BigInt(event.delegationChange.old.toNumber()),
          newDelegatedLamports: BigInt(event.delegationChange.new.toNumber()),
          stakeBalanceWithoutRent,
          staderSolFees: BigInt(event.delegationGrowthStaderSolFees.toNumber()),
          ...
          totalVirtualStakedLamports: BigInt(event.totalVirtualStakedLamports.toNumber()),
          operationalSolBalance: null,
          isDeactivateEvent: false
        });
        
        await this.staderSolPriceHistoryRepository.save(priceHistoryEntry);
        console.log('StaderSolPriceHistory entry saved to Postgres from UpdateActiveEvent');
      });
    } catch (error) {
  ...
  async subscribeToUpdateDeactivatedEvent() {
    try {
      const updateDeactivatedSubId = this.program.addEventListener("UpdateDeactivatedEvent", async (event) => {
        // Save the event to MongoDB
        ...
        const priceHistoryEntry = this.staderSolPriceHistoryRepository.create({
          ...
          stakeIndex: event.stakeIndex,
          stakeAccount: event.stakeAccount.toBase58(),
          oldDelegatedLamports: BigInt(event.lastUpdateDelegatedLamports.toNumber()),
          newDelegatedLamports: null,
          stakeBalanceWithoutRent: BigInt(event.balanceWithoutRentExempt.toNumber()),
          ...
          staderSolSupply: BigInt(event.staderSolSupply.toNumber()),
          totalVirtualStakedLamports: BigInt(event.totalVirtualStakedLamports.toNumber()),
          operationalSolBalance: BigInt(event.operationalSolBalance.toNumber()),
          isDeactivateEvent: true
        });

        await this.staderSolPriceHistoryRepository.save(priceHistoryEntry);
        console.log('StaderSolPriceHistory entry saved to Postgres from UpdateDeactivatedEvent');
      });
    } catch (error) {
  ...
  async subscribeToClaimEvent() {
    try {
      const claimSubId = this.program.addEventListener("ClaimEvent", async (event) => {
        ...
        try {
          await this.stateService.updateUnstakeTicket(event.ticket.toBase58(), {
            claimed: true,
            claimedTime: new Date()
          });
          console.log('Unstake ticket updated in PostgreSQL');
        } catch (error) {
          console.error('Error updating unstake ticket in PostgreSQL:', error);
        }
      });
    } catch (error) {
  ...
  async subscribeToOrderUnstakeEvent() {
    try {
      const orderUnstakeSubId = this.program.addEventListener("OrderUnstakeEvent", async (event) => {
        ...
        unstakeTicket.ticketCreatedEpoch = Number(event.ticketEpoch.toString());
  ...
  async subscribeToAddLiquidityEvent() {
    try {
      const addLiquiditySubId = this.program.addEventListener("AddLiquidityEvent", async (event) => {
  ...
  async subscribeToLiquidUnstakeEvent() {
    try {
      const liquidUnstakeSubId = this.program.addEventListener("LiquidUnstakeEvent", async (event) => {
  ...
  async subscribeToRemoveLiquidityEvent() {
    try {
      const removeLiquiditySubId = this.program.addEventListener("RemoveLiquidityEvent", async (event) => {
  ...
  async subscribeToAddValidatorEvent() {
    try {
      const addValidatorSubId = this.program.addEventListener("AddValidatorEvent", async (event) => {
        ...
        const validatorsAppUrl = this.environmentConfigService.getValidatorsAppUrl();
        const validatorsAppApiKey = this.environmentConfigService.getValidatorsAppKey();
        // const apiResponse = await axios.get(`${validatorsAppUrl}${account}`,{
        //   headers:{Token:validatorsAppApiKey}
        // })
        const validatorData = await this.validatorsDataService.createValidator(event.validator.toString());
        ...
        stakeDelegationEntry.validatorAcIndex = Number(event.index.toString());
  ...
  async subscribeToRemoveValidatorEvent() {
    try {
      const removeValidatorSubId = this.program.addEventListener("RemoveValidatorEvent", async (event) => {
  ...
  async subscribeToSetValidatorScoreEvent() {
    try {
      const setValidatorScoreSubId = this.program.addEventListener("SetValidatorScoreEvent", async (event) => {
  ...
  async subscribeToDepositEvent() {
    try {
      const depositSubId = this.program.addEventListener("DepositEvent", async (event) => {
 ...
 //  * 4. VALIDATOR INDEX  
  async subscribeToDepositStakeAccountEvent() {
    try {
      const depositStakeAccountSubId = this.program.addEventListener("DepositStakeAccountEvent", async (event) => {
 ...
 //  * 4. VALIDATOR INDEX  
  async subscribeToWithdrawStakeAccountEvent() {
    try {
      const withdrawStakeAccountSubId = this.program.addEventListener("WithdrawStakeAccountEvent", async (event) => {
  ...
  async getDelegationRecords(page:number,limit:number){
    try{
      ...
      const [redords,total] = await this.stakeDelegationRepository.findAndCount({
        skip:skip,
        take:limit,
        order:{
          createdAt:'ASC'
      ...
      throw new HttpException('Error while fetching stake delegation records',HttpStatus.BAD_REQUEST);
    }
  }
  ...
  // Admin Events
  async getChangeAuthorityEvents(state?: string, limit: number = 10): Promise<ChangeAuthorityEvent[]> {
    const filter = state ? { state } : {};
    return this.changeAuthorityEventModel.find(filter).sort({ state: -1 }).limit(limit).exec();
  }

  async getConfigLpEvents(state?: string, limit: number = 10): Promise<ConfigLpEvent[]> {
    const filter = state ? { state } : {};
    return this.configLpEventModel.find(filter).sort({ state: -1 }).limit(limit).exec();
  }

  async getConfigStaderLiquidStakingEvents(state?: string, limit: number = 10): Promise<ConfigStaderLiquidStakingEvent[]> {
    const filter = state ? { state } : {};
    return this.configStaderLiquidStakingEventModel.find(filter).sort({ state: -1 }).limit(limit).exec();
  }

  async getEmergencyPauseEvents(state?: string, limit: number = 10): Promise<EmergencyPauseEvent[]> {
    const filter = state ? { state } : {};
    return this.emergencyPauseEventModel.find(filter).sort({ state: -1 }).limit(limit).exec();
  }

  async getInitializeEvents(state?: string, limit: number = 10): Promise<InitializeEvent[]> {
    const filter = state ? { state } : {};
    return this.initializeEventModel.find(filter).sort({ state: -1 }).limit(limit).exec();
  }

  async getReallocStakeListEvents(state?: string, limit: number = 10): Promise<ReallocStakeListEvent[]> {
    const filter = state ? { state } : {};
    return this.reallocStakeListEventModel.find(filter).sort({ state: -1 }).limit(limit).exec();
  }

  async getReallocValidatorListEvents(state?: string, limit: number = 10): Promise<ReallocValidatorListEvent[]> {
    const filter = state ? { state } : {};
    return this.reallocValidatorListEventModel.find(filter).sort({ state: -1 }).limit(limit).exec();
  }

  async getResumeEvents(state?: string, limit: number = 10): Promise<ResumeEvent[]> {
    const filter = state ? { state } : {};
    return this.resumeEventModel.find(filter).sort({ state: -1 }).limit(limit).exec();
  }

  // Crank Events
  async getDeactivateStakeEvents(state?: string, limit: number = 10): Promise<DeactivateStakeEvent[]> {
    const filter = state ? { state } : {};
    return this.deactivateStakeEventModel.find(filter).sort({ state: -1 }).limit(limit).exec();
  }

  async getMergeStakesEvents(state?: string, limit: number = 10): Promise<MergeStakesEvent[]> {
    const filter = state ? { state } : {};
    return this.mergeStakesEventModel.find(filter).sort({ state: -1 }).limit(limit).exec();
  }

  async getRedelegateEvents(state?: string, limit: number = 10): Promise<RedelegateEvent[]> {
    const filter = state ? { state } : {};
    return this.redelegateEventModel.find(filter).sort({ state: -1 }).limit(limit).exec();
  }

  async getStakeReserveEvents(state?: string, limit: number = 10): Promise<StakeReserveEvent[]> {
    const filter = state ? { state } : {};
    return this.stakeReserveEventModel.find(filter).sort({ state: -1 }).limit(limit).exec();
  }

  async getUpdateActiveEvents(state?: string, limit: number = 10): Promise<UpdateActiveEvent[]> {
    const filter = state ? { state } : {};
    return this.updateActiveEventModel.find(filter).sort({ state: -1 }).limit(limit).exec();
  }

  async getUpdateDeactivatedEvents(state?: string, limit: number = 10): Promise<UpdateDeactivatedEvent[]> {
    const filter = state ? { state } : {};
    return this.updateDeactivatedEventModel.find(filter).sort({ state: -1 }).limit(limit).exec();
  }

  // Delayed Unstake Events
  async getClaimEvents(state?: string, limit: number = 10): Promise<ClaimEvent[]> {
    const filter = state ? { state } : {};
    return this.claimEventModel.find(filter).sort({ epoch: -1 }).limit(limit).exec();
  }

  async getOrderUnstakeEvents(state?: string, limit: number = 10): Promise<OrderUnstakeEvent[]> {
    const filter = state ? { state } : {};
    return this.orderUnstakeEventModel.find(filter).sort({ ticketEpoch: -1 }).limit(limit).exec();
  }

  // Liq Pool Events
  async getAddLiquidityEvents(state?: string, limit: number = 10): Promise<AddLiquidityEvent[]> {
    const filter = state ? { state } : {};
    return this.addLiquidityEventModel.find(filter).sort({ solOwner: -1 }).limit(limit).exec();
  }

  async getLiquidUnstakeEvents(state?: string, limit: number = 10): Promise<LiquidUnstakeEvent[]> {
    const filter = state ? { state } : {};
    return this.liquidUnstakeEventModel.find(filter).sort({ staderSolOwner: -1 }).limit(limit).exec();
  }

  async getRemoveLiquidityEvents(state?: string, limit: number = 10): Promise<RemoveLiquidityEvent[]> {
    const filter = state ? { state } : {};
    return this.removeLiquidityEventModel.find(filter).sort({ solLegBalance: -1 }).limit(limit).exec();
  }

  // Management Events

  async getAddValidatorEvents(state?: string, limit: number = 10): Promise<AddValidatorEvent[]> {
    const filter = state ? { state } : {};
    return this.addValidatorEventModel.find(filter).sort({ state: -1 }).limit(limit).exec(); 
  }


  async getRemoveValidatorEvents(state?: string, limit: number = 10): Promise<RemoveValidatorEvent[]> {
    const filter = state ? { state } : {};
    return this.removeValidatorEventModel.find(filter).sort({ state: -1 }).limit(limit).exec();
  }

  async getSetValidatorScoreEvents(state?: string, limit: number = 10): Promise<SetValidatorScoreEvent[]> {
    const filter = state ? { state } : {};
    return this.setValidatorScoreEventModel.find(filter).sort({ state: -1 }).limit(limit).exec();
  }

  // User Events
  async getDepositEvents(state?: string, limit: number = 10): Promise<DepositEvent[]> {
    const filter = state ? { state } : {};
    return this.depositEventModel.find(filter).sort({ solOwner: -1 }).limit(limit).exec();
  }


  async getDepositStakeAccountEvents(state?: string, limit: number = 10): Promise<DepositStakeAccountEvent[]> {
    const filter = state ? { state } : {};
    return this.depositStakeAccountEventModel.find(filter).sort({ stakeIndex: -1 }).limit(limit).exec();
  }

  async getWithdrawStakeAccountEvents(state?: string, limit: number = 10): Promise<WithdrawStakeAccountEvent[]> {
    const filter = state ? { state } : {};
    return this.withdrawStakeAccountEventModel.find(filter).sort({ epoch: -1 }).limit(limit).exec();
  }
}