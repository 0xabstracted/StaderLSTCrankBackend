...
import {
  StaderLiquidStaking,
  IDL,
} from './targets/types/stader_liquid_staking';
...
export class SolanaUtilService {
  ...
  constructor(
    ...
    this.payerKeypair = this.loadKeypairFromFile(
      '../../../secrets/prodDeploy/cranker.json',
    );
    this.wallet = new NodeWallet(this.payerKeypair);
    this.provider = new AnchorProvider(this.connection, this.wallet, {});
    this.program = new Program<StaderLiquidStaking>(
      IDL,
      this.programId,
      this.provider,
    );
    this.crankerKeypair = this.loadKeypairFromFile(
      '../../../secrets/prodDeploy/cranker.json',
    );
    this.stateAccountPublicKey = new PublicKey(
      '6WkY9wdYRVcU1kEFwsKy1DEXSQtYw9qViu4d6Phi6cYB',
    );
    this.stakeListPublicKey = new PublicKey(
      'A8NUYkzHwPyWBQcoWgNYjAsVFWAinj3SRJwG9VBjxMZH',
    );
    this.validatorListPublicKey = new PublicKey(
      'B8jPF5FsjJRteUeSNqd4X7BucubTHnXutRZqphdfSYUr',
    );
    this.operationalSolAccountPublicKey = new PublicKey(
      '6hhhBVVJmaxdqJCSMYP44kooweDrHoStkPQo5wyPrN1s',
    );
    this.staderSolMintPublicKey = new PublicKey(
      'sTadM6ZhQGr9e45fpV6ig92X1ZEELRVjAiJ8qJ9SLXQ',
    );
    this.lpMintPublicKey = new PublicKey(
      'HnFze3Ef6hY48HMyVyHkRTwSrPWmf7e35JeEqtDaEELC',
    );
    [this.staderSolMintAuthority] = PublicKey.findProgramAddressSync(
      [this.stateAccountPublicKey.toBuffer(), Buffer.from('st_mint')],
      this.programId,
    );
    [this.authorityLpAcc] = PublicKey.findProgramAddressSync(
      [this.stateAccountPublicKey.toBuffer(), Buffer.from('liq_mint')],
      this.programId,
    );
    [this.reservePda] = PublicKey.findProgramAddressSync(
      [this.stateAccountPublicKey.toBuffer(), Buffer.from('reserve')],
      this.programId,
    );
    [this.solLegPda] = PublicKey.findProgramAddressSync(
      [this.stateAccountPublicKey.toBuffer(), Buffer.from('liq_sol')],
      this.programId,
    );
    [this.authorityStaderSolLegAcc] = PublicKey.findProgramAddressSync(
      [
        this.stateAccountPublicKey.toBuffer(),
        Buffer.from('liq_st_sol_authority'),
      ],
      this.programId,
    );
    [this.stakeDepositAuthority] = PublicKey.findProgramAddressSync(
      [this.stateAccountPublicKey.toBuffer(), Buffer.from('deposit')],
      this.programId,
    );
    [this.stakeWithdrawAuthority] = PublicKey.findProgramAddressSync(
      [this.stateAccountPublicKey.toBuffer(), Buffer.from('withdraw')],
      this.programId,
    );
    this.treasuryStaderSolAccount = getAssociatedTokenAddressSync(
      this.staderSolMintPublicKey,
      this.stateAccountPublicKey,
      true,
    );
    this.staderSolLeg = getAssociatedTokenAddressSync(
      this.staderSolMintPublicKey,
      this.authorityStaderSolLegAcc,
      true,
    );
  }
  ...
  public loadKeypairFromFile(fileName: string): Keypair {
    ...
    try {
      ...
      this.logger.error(
        'Error while loading keypair. Ensure the file exists and is readable.',
        error,
      );
      throw new Error(
        'Keypair file is missing or inaccessible. Check the file path and permissions.',
      );
    }
  }
  ...
  private initializeRPCClientConnect() {
    if (this.environmentConfigService.getSolanaCluster() === 'MAINNET') {
      this.connection = new Connection(
        this.environmentConfigService.getSolanaMainnetRPCUrl(),
        'confirmed',
      );
    } else {
      this.connection = new Connection(
        this.environmentConfigService.getSolanaDevnetRPCUrl(),
        'confirmed',
      );
    }
  }
