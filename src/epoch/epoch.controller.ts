...
export class EpochController {
  ...
  @ApiQuery({
    ...
    required: false,
    type: Number,
    description: 'Target epoch number. If not provided, calculates for next epoch'
  })
  @ApiHeader({
    name: 'x-api-key',
    description: 'API key for authentication',
  ...
  async getEpochInfo(@Query('targetEpoch', new ParseIntPipe({ optional: true })) targetEpoch?: number) {
    try {
      ...
      try {
        ...
        if (!epochToFetch) {
          const currentInfo = await this.epochTrackerService.getEpochInfoFromDb();
          epochToFetch = currentInfo.epoch + 1;
        }
        
        const epochInfo = await this.epochTrackerService.getEpochInfoFromDb(epochToFetch);
  ...
  @ApiQuery({
    ...
    required: false,
    type: Number,
    description: 'Target slot number. If not provided, returns current slot info'
  })
  @ApiHeader({
    name: 'x-api-key',
    description: 'API key for authentication',
  ...
  async getSlotInfo(@Query('slot', new ParseIntPipe({ optional: true })) slot?: number) {
    try {
