...
export class EpochTrackerService implements OnModuleInit {
  ...
  async initializeEpochInfo() {
    try {
      ...
      if (!existingEpoch) {
        ...
        this.logger.log(
          `Epoch info initialized for epoch ${epochProgress.currentEpoch}`,
        );
      } else {
        this.trackedEpoch = existingEpoch.epoch;
        this.logger.log(
          `Epoch info already exists for epoch ${existingEpoch.epoch}`,
        );
      ...
      this.logger.error(
        `Error initializing epoch info: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  private async createNextEpochInfo(
    ...
    try {
      ...
      const endTimestamp =
        startTimestamp + slotsInEpoch * this.epochService.SLOT_TIME_SECONDS;
      ...
      this.logger.error(
        `Error creating next epoch info: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async checkEpochChange() {
    try {
      ...
      if (
        ...
        epochProgress.currentEpoch !== this.trackedEpoch
      ) {
        this.logger.log(
          `Epoch change detected. Previous: ${this.trackedEpoch}, Current: ${epochProgress.currentEpoch}`,
        );
        ...
        if (existingNewEpoch) {
          ...
          this.logger.log(
            `Updated epoch ${epochProgress.currentEpoch} to active`,
          );
        } else {
          ...
          await this.epochInfoRepository.save(newEpochInfo);
          this.logger.log(
            `Created epoch info for new epoch ${epochProgress.currentEpoch}`,
          );
        }
        ...
        this.logger.debug(
          `No epoch change detected. Current epoch: ${epochProgress.currentEpoch}`,
        );
      }
    } catch (error) {
      this.logger.error(
        `Error checking for epoch change: ${error.message}`,
        error.stack,
      );
    }
  }
  ...
  async getEpochInfoFromDb(targetEpoch?: number): Promise<any> {
    try {
      ...
      if (!epochInfo) {
        throw new Error(
          `Epoch info not found for ${targetEpoch || 'active epoch'}`,
        );
      }
      ...
      this.logger.error(
        `Error fetching epoch info from DB: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async calculateEpochProgress(): Promise<{
    ...
    try {
      ...
      this.logger.error(
        `Error calculating epoch progress: ${error.message}`,
        error.stack,
      );
      throw error;
    }
